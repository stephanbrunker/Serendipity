...
class serendipity_event_multilingual extends serendipity_event
{
    ...
    function introspect(&$propbag)
    {
        ...
        // get multilanguage strings for all used languages
        $langs = $this->getUsedLangs();
        unset($langs[$serendipity['lang']]); // already loaded
        $charset = serendipity_getCharset();
        foreach ($langs as $lang) {
            include(S9Y_INCLUDE_PATH . 'lang/' . $charset . 'serendipity_langvar_'. $lang .'.inc.php');
        }
    }
    ...
    static function strip_langs($msg, $lang = '')
    {
        global $serendipity;

        if (!preg_match('@{{@', $msg)) return $msg;

        if (empty($lang)) $lang = $serendipity['lang'];
    ...
    function getUsedLangs() {
        ...
        if ($serendipity['dbType'] == 'postgres' || $serendipity['dbType'] == 'pdo-postgres') {
            ...
            $cond['group']    = 'GROUP BY lang';
            $cond['distinct'] = '';
        }
        $cond['where'] = 1;
        $langsubscribers = array();
        if (version_compare($serendipity['versionInstalled'], '2.4.alpha2' , '>')) {
            $sql = "SELECT {$cond['distinct']} lang FROM {$serendipity['dbPrefix']}subscriptions WHERE {$cond['where']} {$cond['group']}";
            $res = serendipity_db_query($sql, false, 'assoc');
            foreach ($res as $r) {
                $langsubscribers[] = $r['lang'];
            }
        }

        // languages of entries
        if ($serendipity['dbType'] != 'postgres' && $serendipity['dbType'] != 'pdo-postgres') {
            $cond['group']    = 'GROUP BY property';
        }
        $cond['where'] = "property LIKE '%multilingual_%'";
        $sql = "SELECT {$cond['distinct']} property FROM {$serendipity['dbPrefix']}entryproperties WHERE {$cond['where']} {$cond['group']}";
        $entryproperties = serendipity_db_query($sql, false, 'assoc');
        $pattern = '@multilingual_(title|body|extended)_([A-Za-z_]+)@';
        $matches = array();
        $langentries = array();
        foreach ($entryproperties as $props) {
            preg_match($pattern, $props['property'], $matches);
            $langentries[] = $matches[2];
        }
        if (is_array($langentries)) {
            $langentries = array_unique($langentries);
        }

        // languages of users
        $cond['where'] = "name = 'lang'";
        if ($serendipity['dbType'] != 'postgres' && $serendipity['dbType'] != 'pdo-postgres') {
            $cond['group']    = 'GROUP BY value';
        }
        $sql = "SELECT {$cond['distinct']} value FROM {$serendipity['dbPrefix']}config WHERE {$cond['where']} {$cond['group']}";
        $res = serendipity_db_query($sql, false, 'assoc');
        $langusers = array();
        foreach ($res as $r) {
            $langusers[] = $r['value'];
        ...
        // combine these three arrays
        $langs = array_unique(array_merge($langsubscribers, $langentries, $langusers));
        ...
        if (isset($hooks[$event])) {
            switch($event) {
                ...
                case 'genpage':
                    ...
                    // modifier should be obsolete, because all data ist stripped before passing to smarty
                    if ($serendipity['version'][0] < 2) {
...
<?php
                ...
                case 'mail_subscribers':
                    ...
                    $mail = $eventData;
                    foreach ($langs as $lang) {
                        if (!array_key_exists($lang, $mail)) {
                            if (!empty($entry['entryproperties']['multilingual_body_' . $lang]) 
                                    && !empty($entry['entryproperties']['multilingual_title_' . $lang])) {
                                $entry['body'] = $entry['entryproperties']['multilingual_body_' . $lang];
                                $entry['title'] = $entry['entryproperties']['multilingual_title_' . $lang];
                                $entry['extended'] = $entry['entryproperties']['multilingual_extended_' . $lang];
                                $mail[$lang] = serendipity_generateSubscriptionMail($entry, $lang);
                            }
